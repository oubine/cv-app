...
- [3. Développement du site web]("#developpement")
	...
	- [c. Outils et librairies utilisés]("#librairies")
...
Ce projet de site web intitulé: *"Conception ergonomique, Design et Développement Web d'un CV interactif sur internet"* entre dans le cadre d'une TZ. À l'UTC, une TZ est l'appellation pour un projet de fin d'étude: les étudiants en fin de cursus à qui il manque des crédits pour valider leur diplôme peuvent réaliser un projet en autonomie pour rattraper leurs derniers crédits manquants sans avoir à faire un semestre supplémentaire. 

C'est la situation dans laquelle je me suis retrouvée en février. J'ai alors décidé de réaliser un projet orienté web car je souhaite m'orienter vers le design d'interface utilisateur pour mon parcours professionnel. Le but de mon projet était de créer un site web faisant office de CV utilisable par la suite dans ma vie professionnelle.

J'ai assuré toutes les étapes de création de ce site web. Ce processus se déroule en 3 étapes: 

-  le design d'interface;
-  le développement;
-  la mise en ligne. 

La première étape consiste à conceptualiser l'interface utilisateur, la seconde consiste à coder cette interface et enfin la mise en ligne revient à rendre le site disponible à tous sur internet. Les outils utilisés pour chaque étape sont détaillés dans les parties dédiées dans la suite du rapport. Néanmoins, ces étapes ne sont pas indépendantes les unes aux autres. 

En effet, dans la mesure où j'ai été la seule à travailler sur ce projet, je n'ai pas suivi ce processus linéairement, en réalisant les étapes les unes après les autres. Par exemple, la conception de certains éléments de l'interface a été faite en même temps que le développement de l'application. Dans un milieu professionnel, différentes personnes sont en charge des différentes étapes du processus, donc les tâches sont plus divisées. Cependant, pour un souci de clarté, les différentes étapes du processus sont expliquées linéairement dans la suite du rapport.

## 2. Design de l'interface<a id="design"></a>

Donc, dans un premier temps, il a fallu procéder au design de l'interface et répondre aux questions suivantes : **Quelle est la fonction de ce site? Quelles sont les informations nécessaires dans un CV? Quelles sont les informations les plus importantes à mettre en avant et comment les agencer entre elles?  Quelles sont les fonctionnalités nécessaires pour rendre la lecture de ce CV dynamique et interactive ? Quelles couleurs utiliser?** Après avoir réfléchi à toutes ces questions, il faut faire une maquette du site, c'est-à-dire créer des visuels qui montrent à quoi le site devrait ressembler à la fin du projet. Ces aperçus ne sont pas fonctionnels mais à but indicatifs pour donner une direction au travail de développement. 

### a. Outils utilisés<a id="outils_utilisés"></a>

Pour cette étape je me suis tout d'abord basée sur un CV (téléchargeable sur mon site) que j'avais réalisé pour ma recherche de TN10 (appellation du stage de fin d’étude à l’UTC). Ainsi, j'ai rapidement su quelles étaient les informations à faire figurer sur le site. 
Pour réaliser ces maquettes, j'ai utilisé figma, un logiciel est un éditeur de graphiques vectoriels et un outil de prototypage, dédié à l'élaboration de maquettes d'interfaces graphiques de site web. 

### b. Organisation des informations et choix de la charte graphique<a id="charte_graphique"></a>

Un site est divisé en plusieurs sections ou pages pour diviser et classer les informations en fonction de leur type. Pour définir les différentes sections de mon CV en ligne, j'ai repris les différents paragraphes de mon CV et je les ai adaptés. Ainsi, mon CV en ligne comporte 4 sections distinctes qui sont les suivantes:
...
Cependant, le but de ce projet n'est pas de faire une retranscription de mon CV "papier" sur un écran. En effet, un site web offre des possibilités d'interactivité et de design bien plus larges qu'un simple design statique. Ainsi, j'ai pu choisir de regrouper plusieurs types d'informations sous une même section. Par exemple, la section "loisirs", qui apparaît traditionnellement sur les CV et qui me semble peu importante, a été greffée à la section "contacts" dans laquelle je parle un peu plus de moi. 

De plus, j'ai ajouté une page d'accueil et un menu. Ce sont des éléments qu'un utilisateur s’attend à voir lorsqu' il navigue sur internet et qui sont donc importants pour que ce dernier retrouve des repères pour naviguer naturellement sur le site. J'ai également choisi d'ajouter quelques interactions et animations pour rendre le site plus dynamique et attractif. 

Enfin, il a fallu que je choisisse une charte graphique. La charte graphique d'un site est l'identité visuelle de ce dernier: c'est ce qui fait qu'un site est reconnaissable. Elle se compose de polices, de couleurs et de certaines normes visuelles. Je voulais un site moderne et épuré, c'est pourquoi j'ai mis peu d'éléments "décoratifs" dans ma charte graphique. En effet, les seuls éléments à but purement décoratifs sont des encadrements autour des images et quelques lignes fines. J'ai préféré miser sur:
...
- les interactions: aujourd'hui les sites sont très interactifs pour être plus attractifs.  
...
Cette commande lance en local les scripts du dossier depuis lequel elle a été lancée. Ainsi, quand cette commande est lancée, toutes les modifications apportées aux scripts sont visibles à l'adresse: http://localhost:3000/. 
...
- @brainhubeu/react-carousel: pour le caroussel dans la partie **Projets** de l'application;
